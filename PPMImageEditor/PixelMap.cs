...
namespace PPMImageEditor
{
  ...
  public class PixelMap
  {
    ...
    private FSharpList<FSharpList<Tuple<int, int, int>>> imageListData;
    /// <summary>
    /// The data portion of the PixelMap, in list of lists form.
    /// </summary>
    public FSharpList<FSharpList<Tuple<int,int,int>>> ImageListData
    {
      get
      {
        if (imageListData == null)  // first request, populate:
        {
          imageListData = FSharpList<FSharpList<Tuple<int, int, int>>>.Empty;
          ...
          for (int i = rawImageData.Length; i > 0; /*nop*/)
          {
            ...
            for (int j = Stride; j > 0; /*nop*/)  // for each pixel in row:
            {
              i--;
              int B = rawImageData[i];
              i--;
              int G = rawImageData[i];
              i--;
              int R = rawImageData[i];

              System.Tuple<int, int, int> T = Tuple.Create(R, G, B);
            ...
            imageListData = new FSharpList<FSharpList<Tuple<int, int, int>>>(row, imageListData);
          }//for
        }
